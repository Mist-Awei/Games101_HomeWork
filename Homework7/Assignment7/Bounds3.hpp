                                ...
                                const std::array<int, 3>& dirIsNeg) const
{
    ...
    /*
    Vector3f tMin = Vector3f::Min(t1, t2);//’“µΩtMin∫œºØ
    ...
    */
    ...
    if (dir.x < 0) std::swap(t1.x, t2.x);
    if (dir.y < 0) std::swap(t1.y, t2.y);
    if (dir.z < 0) std::swap(t1.z, t2.z);
    float tEnter = fmax(t1.x, fmax(t1.y, t1.z)), tExit = fmin(t2.x, fmin(t2.y, t2.z));
